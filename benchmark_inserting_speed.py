...
def benchmark_sqlalchemy_core(dburl="sqlite:///sqlalchemy.db", n=1000_000):
    sqlalchemy_base, sqlalchemy_engine = get_sqlalchemy_base_engine(dburl=dburl)

    columns = [Column("id", Integer, primary_key=True)] + [
        Column(colname, String(255)) for colname in ["name"]
    ]
    ...
    sqlalchemy_engine.execute(
        table.insert(), [{"name": "NAME " + str(i)} for i in range(n)]
    )
    print(
        "database-url '%s' : inserting %d records took: %0.2f secs"
        % (dburl, n, time.time() - t0)
    )


if __name__ == "__main__":

    benchmark_sqlalchemy_core(
        dburl="postgresql://postgres:postgres@localhost:5432/postgres"
    )
    benchmark_sqlalchemy_core(
        dburl="mysql+mysqlconnector://mysql:mysql@localhost:3306/mysql"
    )
