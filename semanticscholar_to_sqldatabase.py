    ...
    def consumer_supplier():  ## called once on worker
        ...
        def consumer(file: str):  ## called multiple times on worker
            with sqlalchemy_engine.connect() as conn:
                ...
                print(
                    "%s skipping %d lines in file %s"
                    % (multiprocessing.current_process(), num_to_skip, file)
                )
                data_g = (
                    fill_missing_with_Nones(d, column_names)
                ...
                for count in populate_table(conn, table, data_g, batch_size=batch_size):
                    conn.execute(
                        state_table.update()
                        .values(line=count + num_to_skip)
                        .where(state_table.c.file == file)
                    )
        ...
        def num_lines_already_read(conn, file):
            num_to_skip = next(
                get_rows(conn, select([state_table]).where(state_table.c.file == file))
            )["line"]
            return num_to_skip
...
def create_or_expand_state_table(
    sqlalchemy_base, sqlalchemy_engine, table_name, files: List[str], from_scratch=False
):
    tables = get_tables_by_reflection(sqlalchemy_base.metadata, sqlalchemy_engine)
...
def build_state_table(sqlalchemy_base, state_table_name):
    ...
    state_table = Table(
        state_table_name, sqlalchemy_base.metadata, *columns, extend_existing=True
    )
    return state_table


def run_table_population(
    build_table,
    files,
    dburl,
    num_processes=1,
    num_to_insert=10_000,
    ...
    state_table = create_or_expand_state_table(
        sqlalchemy_base,
        sqlalchemy_engine,
        table_name,
        files,
    ...
    files = [
        f[0]
        for f in sqlalchemy_engine.execute(
            select([state_table.c.file]).where(state_table.c.done == False)
        )
    ]
    ...
    start = time()
    if num_processes == 1:
        consumer = build_consumer_supplier(
            table_name, limit=num_to_insert, batch_size=batch_size, dburl=dburl
        )()
        [consumer(file) for file in files]
    else:
        pool_consume(
            data=files,
            consumer_supplier=build_consumer_supplier(
                table_name, limit=num_to_insert, batch_size=batch_size, dburl=dburl
            ),
    ...
    if benchmark_mode:
        ...
        print(
            "with %d processes inserted %d rows in %0.2f secs with speed of %0.2f rows/sec"
...
if __name__ == "__main__":
    ...
    def build_table(sqlalchemy_base):
        ...
        columns = [Column("id", String, primary_key=True)] + [
            Column(colname, String(), nullable=True) for colname in column_names
        ]
        table = Table(
            table_name, sqlalchemy_base.metadata, *columns, extend_existing=True
        )
        return table
    ...
    df = pandas.DataFrame(
        data=[{"num-cores": n, "speed": benchmark_fun(n)} for n in [1, 2, 4, 8, 12]]
    )
