...
def insert_or_update(
    ...
    table: Table,
    columns_to_update,
    rows_g: Iterable[Dict],
    update_fun=default_update_fun,
    batch_size=10000,
):
    def fun(batch):
...
def insert_or_update_batch(
    ...
    columns_to_update,
    rows: List[Dict],
    process_val_fun=default_update_fun,
):
    ids2values = {d.pop("id"): d for d in rows}

    rows_to_update = [
        d
        for d in conn.execute(select([table]).where(table.c.id.in_(ids2values.keys())))
    ]
    ...
    if len(ids_to_insert) > 0:
        conn.execute(
            table.insert(),
            [
                {**{"id": eid}, **process_val_fun(old_row=None, val=ids2values[eid])}
                for eid in ids_to_insert
            ],
        )
    ...
    if len(rows_to_update) > 0:
        stmt = (
            ...
            .values(
                **{
                    col_name: bindparam("val_" + col_name)
        ...
        def build_dict_of_processed_values(old_row):
            return {
                "val_" + col: processed_val
                for col, processed_val in process_val_fun(
                    ids2values[old_row["id"]], old_row=old_row
                ).items()
            }
        ...
        conn.execute(
            stmt,
            [
                {**{"obj_id": d["id"]}, **build_dict_of_processed_values(d)}
                for d in rows_to_update
            ],
        )


def process_table_batchwise(
    sqlalchemy_engine,
    q: Query,
    table: Table,
    ...
    num_processes=0,
    initializer_fun=None,
    initargs=(),
):
    batch_generator = fetchmany_sqlalchemy(
        sqlalchemy_engine, q, batch_size=batch_size, stop_fun=stop_fun
    )
    process_time = 0
    if num_processes > 0:
        with sqlalchemy_engine.connect() as conn:
            with Pool(
                processes=num_processes, initializer=initializer_fun, initargs=initargs
            ) as pool:
                for processed_batch, dur in iterate_and_time(
                    pool.imap_unordered(process_batch_fun, batch_generator)
                ):
                    process_time += dur
...
def update_table(
    conn, processed_batch: List[Dict[str, str]], table: Table, primary_key_col="id"
):
...
def insert_or_overwrite(conn, table: Table, rows: List[Dict]):
    ...
    if len(ids_to_insert) > 0:
        conn.execute(
            table.insert(),
            [{**{"id": eid}, **ids2values[eid]} for eid in ids_to_insert],
        )
    ...
    if len(ids_to_overwrite) > 0:
        stmt = (
            ...
            .where(table.c.id == bindparam("obj_id"))
            .values(
                **{col_name: bindparam("val_" + col_name) for col_name in column_names}
            )
        )
        conn.execute(
            stmt,
            [
                {
                    **{"obj_id": eid},
                    **{"val_" + c: ids2values[eid][c] for c in column_names},
                }
                for eid in ids_to_overwrite
            ],
        )


def insert_if_not_existing(
    conn, table: Table, data: Iterable, primary_key_col="id", batch_size=10000
):
    def insert_batch(rows):
        ...
        existing_ids = set(
            [
                ...
                for d in conn.execute(
                    select([table]).where(getattr(table.c, primary_key_col).in_(ids))
                )
            ]
        )
        ...
        if len(ids_to_insert) > 0:
            conn.execute(
                table.insert(), [d for d in rows if d[primary_key_col] in ids_to_insert]
            )
...
def fetchmany_sqlalchemy(
    sqlalchemy_engine, query: Query, batch_size=10000, stop_fun=lambda: False
):
